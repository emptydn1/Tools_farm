            ...
            const p = (async () => {
                ...
                try {
                    while (!isKilled) {
                        ...
                        const image_to_doi = base.clone()
                            .extract({ left: 10, top: 210, width: 220, height: 70 });

                        if (task1) image_to_doi.resize({ width: 220 * 10, height: 70 * 10 })
                        ...
                        if (findLocation(ocrText)?.city && /0\s*\/\s*1/.test(ocrTextToDoi)) {
                            await tap(host, 100, 200)
                        } else if (findLocation(ocrText, 1)?.city && task1 == false) {
                            await tap(host, 100, 200)
                            await tap(host, 100, 200)
                            await tap(host, 100, 200)
                            await sleep(1000)
                            ...
                            while (loop) {
                                ...
                                const { matchedPoints } = await findMatchingRegionsAndroids({
                                    buffer: pngBuffer,
                                    templateImages: [
                                        'C:\\Users\\huy\\Desktop\\Tools_farm\\b1.png',
                                        'C:\\Users\\huy\\Desktop\\Tools_farm\\b4.png',
                                    ],
                                    matchThreshold: 0.8,
                                });
                                ...
                                if (matchedPoints.length > 0) {
                                    for (const { x, y, mathImagePath } of matchedPoints) {
                                        if (mathImagePath == 'C:\\Users\\huy\\Desktop\\Tools_farm\\b1.png') {
                                            ...
                                            await to_doi(host, ocrTextToDoi)
                                        } else if (mathImagePath == 'C:\\Users\\huy\\Desktop\\Tools_farm\\b4.png') {
                                            loop = false;
                                            await tap(host, 60, 155)    // click tab nhiem vu
                                            await tap(host, 60, 155)    // click tab nhiem vu
                                        }
                        ...
                        } else if (/1\s*\/\s*1/.test(ocrTextToDoi)) {
                            await tap(host, 100, 200)
                            await tap(host, 100, 200)
                            await tap(host, 100, 200)
                            ...
                            const { matchedPoints } = await findMatchingRegionsAndroids({
                                buffer: pngBuffer,
                                templateImages: [
                                    'C:\\Users\\huy\\Desktop\\Tools_farm\\b2.png',
                                    'C:\\Users\\huy\\Desktop\\Tools_farm\\b3.png',
                                ],
                                matchThreshold: 0.8,
                            });
                            ...
                            if (matchedPoints.length > 0) {
                                for (const { x, y, mathImagePath } of matchedPoints) {
                                    if (mathImagePath == 'C:\\Users\\huy\\Desktop\\Tools_farm\\b2.png') {
                                        await logout_and_login(host)
                                        return;
                                    } else if (mathImagePath == 'C:\\Users\\huy\\Desktop\\Tools_farm\\b3.png') {
                                        await logout_and_login(host)
                                        return;
                                    }
                                }
