...
import { waitForInput, sleep, printFormattedTitle } from './utils/utils.js';
...
(async () => {
    ...
    while (tasks.length > 0) {
        ...
        await Promise.all(currentBatch.map(async (e, index) => {
            ...
            printFormattedTitle(`account ${e} - Profile ${e + 100} - proxy null`, "red");
            await MainBrowser(e, null, positionX, positionY);
        }));
        runCore = undefined;
    }
    process.exit(0);
