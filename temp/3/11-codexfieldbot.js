...
const MainBrowser = async (userProfileIndex, proxy, positionX, positionY) => {
    try {
        let { chrome, cursor, cursorSession, client, cursorActionClient, sessionIframe, cursorActionIframe, iframes, workers } = await runChrome({
            ...
            url: "https://web.telegram.org/k/#@codexfieldbot",
            // isMobile: true,
        });
        ...
        // checkDaily
        await cursorActionIframe.moveToSelector({ selector: "#root > div > div > div.flex.flex-1.flex-col.w-full.justify-between.text-white.py-8.duration-1000.opacity-100 > div.flex.flex-col.items-center.px-8.mt-3 > button", iframe: client });
        await cursorActionIframe.moveToSelector({ selector: "#root > div > div > div.flex.flex-1.flex-col.w-full.justify-between.text-white.py-8.duration-1000.opacity-100 > div.flex.flex-col.items-center.px-8.mt-3 > button", maxWaitTime: 2000, iframe: true });
