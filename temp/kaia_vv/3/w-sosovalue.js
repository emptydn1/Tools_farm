...
const MainBrowser = async ({ userProfileIndex, proxy, positionX, positionY }) => {
    try {
        let { chrome, cursor, cursorSession, client, cursorActionClient, sessionIframe, cursorActionIframe, iframes, workers } = await runChrome({
            ...
            url: "https://sosovalue.com/join/8O32AVCL",
            url: "https://sosovalue.com/exp",
        ...
        await client.Page.navigate({ url: 'https://google.com' });
        await sleep(5000);
        await client.Page.navigate({ url: 'https://sosovalue.com/exp' });
        ...
        // await cursorActionClient.moveToSelector({
        //     selector: {
        //         script: `(() => {
        //         let rect = document.querySelector("#root > div > div > div > div.overflow-x-auto > div > div > div.relative.pl-\\\\[89px\\\\].pt-\\\\[113px\\\\].pb-0.h-\\\\[calc\\\\(100\\\\%-50px\\\\)\\\\] > div > div > div.h-full.overflow-y-auto.p-6.space-y-6.flex-1.expLg\\\\:border-0.expLg\\\\:border-neutral-stroke-1-rest.expLg\\\\:border-l.expLg\\\\:border-solid.overflow-hidden > div.grid.xl\\\\:grid-cols-3.grid-cols-1.gap-4").children[0].querySelector('button')?.getBoundingClientRect();
        ...
        // let a = document.querySelector("#root > div > div > div > div.overflow-x-auto > div > div > div.relative.pl-\\\\[89px\\\\].pt-\\\\[113px\\\\].pb-0.h-\\\\[calc\\\\(100\\\\%-50px\\\\)\\\\] > div > div > div.h-full.overflow-y-auto.p-6.space-y-6.flex-1.expLg\\\\:border-0.expLg\\\\:border-neutral-stroke-1-rest.expLg\\\\:border-l.expLg\\\\:border-solid.overflow-hidden > div.grid.xl\\\\:grid-cols-3.grid-cols-1.gap-4").children[${i}].querySelector('button');
        ...
        //         selector: {
        //             script: `(() => {
        //             let rect = document.querySelector("#root > div > div > div > div.overflow-x-auto > div > div > div.relative.pl-\\\\[89px\\\\].pt-\\\\[113px\\\\].pb-0.h-\\\\[calc\\\\(100\\\\%-50px\\\\)\\\\] > div > div > div.h-full.overflow-y-auto.p-6.space-y-6.flex-1.expLg\\\\:border-0.expLg\\\\:border-neutral-stroke-1-rest.expLg\\\\:border-l.expLg\\\\:border-solid.overflow-hidden > div.grid.xl\\\\:grid-cols-3.grid-cols-1.gap-4").children[${i}].querySelector('button')?.getBoundingClientRect();
        //             if (rect && rect.width > 0) return JSON.parse(JSON.stringify(rect));
        //             throw "error";
        //         })()`
        //         },
