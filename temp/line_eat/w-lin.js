...
const MainBrowser = async ({ userProfileIndex, proxy, positionX, positionY, index }) => {
    try {
        ...
        const UA_LIST = fs.readFileSync('C:\\Users\\huy\\Desktop\\Tools_Farm\\core\\user.txt', 'utf8').split('\n').map(line => line.trim()).filter(line => line.length > 0);
        ...
        rootEmpty.push(cursorActionTab);
        ...
        const bot = new GameBot(tab, cursorActionTab);
                ...
                if (matchedPoints.length > 0) {
                    for (const { x, y, mathImagePath } of matchedPoints) {
                        if (mathImagePath == 'C:\\Users\\huy\\Desktop\\Tools_Farm\\match-img\\lin\\connect.jpeg') {
                            ...
                            await bot.clickPoint(193, 429)
                            await bot.clickPoint(193, 429)
                            await bot.clickPoint(144, 431)
                            await bot.clickPoint(255, 399)
                        }
                    }
                }
            } catch (error) {
...
class GameBot {
    constructor(client, cursorActionTab = null) {
        this.client = client;
        this.cursorActionTab = cursorActionTab;
    }
    ...
    // Click vào 1 điểm
    async clickPoint(x, y, delay = 0) {
        try {
            await this.sleep(delay);
            await this.cursorActionTab.moveTo({ x, y })
        } catch (err) {
            console.log(err)
        }
    }
    ...
    async clickUpgrade() {
        for (let i = 0; i < 6; i++) {
            await this.clickPoint(443, 455, 100);
        }
    }
...
async function runTask(a, b) {
    const mouse = async (type, x, y, opts = {}) => await rootEmpty[a].moveTo({ x, y })
